...
const SigItemRecipe = ({ quest_1, sig_item, sig_stats, forge_bonus }) => {
  return (
    <div className="sig-item-recipe">
      <div id="sig-item" className="item-icon">
        ...
        <div className="tooltip">
          ...
          <p>
            {`${formatText(forge_bonus[0].stat)} ≥ 40 ${formatText(
              forge_bonus[0].bonus
            )}`}
            <span className="grn-txt">{` + ${forge_bonus[0].value}`}</span>
          </p>
          <p>
            {`${formatText(forge_bonus[1].stat)} ≥ 100 ${formatText(
              forge_bonus[1].bonus
            )}`}
            <span className="grn-txt">{` + ${forge_bonus[1].value}`}</span>
          </p>
          <p>
            {`${formatText(forge_bonus[2].stat)} ≥ 200 ${formatText(
              forge_bonus[2].bonus
            )}`}
            <span className="grn-txt">{` + ${forge_bonus[2].value}`}</span>
          </p>
        </div>
      </div>
      <div id="aw-item1" className="item-icon">
        <img
          src={`/assets/images/icons/signature-items/quest1/${quest_1.fragment}.png`}
          alt="item icon"
        />
      ...
      <div id="aw-item2" className="item-icon">
        <img
          src={`/assets/images/icons/equipment/${quest_1.recipe[0]}.png`}
          alt="item icon"
        />
      ...
      <div id="aw-item3" className="item-icon">
        <img
          src={`/assets/images/icons/equipment/${quest_1.recipe[1]}.png`}
          alt="item icon"
        />
      ...
      <div id="aw-item4" className="item-icon">
        <img
          src={`/assets/images/icons/equipment/${quest_1.recipe[2]}.png`}
          alt="item icon"
        />
